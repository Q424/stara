...
void CAirFountain::Render(vector3 cp)
{

    float xDist = cp.x - EmitterPos.x;
    float zDist = cp.z - EmitterPos.z;
...
//  glDisable(GL_CULL_FACE);
    ...
    glColor4f(color.r, color.g, color.b, color.a-0.4);
  ...
  //glEnable(GL_POINT_SMOOTH);
	...
	glVertexPointer(3, GL_FLOAT, 0, FountainVertices);
      ...
      //glBindTexture(GL_TEXTURE_2D, 0);
        glPushMatrix();
        glTranslatef(EmitterPos.x, EmitterPos.y, EmitterPos.z);
	glDrawArrays(GL_POINTS,	0, m_NumDropsComplete);
        glPopMatrix();
