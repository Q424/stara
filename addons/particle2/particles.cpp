...
void CCCParticleSystem::SetUseTex(bool state)
{
	m_bUseTexture = state;
}
//*********************************************************
...
{
 WriteLog("LoadTextureFromFile " + AnsiString(Filename));
...
void CCCParticleSystem::Render(bool a, GLuint tex)
{
   ...
   //      glDisable(GL_LIGHTING);
		...
		if (!a) m_Texture->SetActive();
                if ( a) glBindTexture(GL_TEXTURE_2D, tex);
