...
{ // ABu290105: pozbierane i uporzadkowane powtarzajace sie rzeczy z Render i RenderAlpha
    ...
    if (ObjSqrDist < 160000) // gdy bli¿ej ni¿ 400m
    {
        ...
        if (mdSTATIC10) mdSTATIC10->GetSMRoot()->SetTranslate(modelShake);
            ...
            // nie przewody powietrzne, yB: decyzja na podstawie polaczen w t3d
            ...
            for (int i = 0; i < Track->iNumDynamics; i++)
            {
                if (Track->Dynamics[i] != this)
                {
                    TestDist = MyTranslation -
                        (Track->Dynamics[i]->RaTranslationGet()); //???-przesuniêcie wózka wzglêdem Point1 toru
                    if ((TestDist > 0) && (TestDist < MinDist))
                    {
                        CouplFound = (Track->Dynamics[i]->RaDirectionGet() > 0) ?
                                         0 :
...
{ // Ustawienie pocz¹tkowe pojazdu
    ...
    */
    ...
    iLights = MoverParameters->iLights; // wskaŸnik na stan w³asnych œwiate³ (zmienimy dla rozrz¹dczych EZT) McZapkie: TypeName musi byc nazw¹ CHK/MMD pojazdu
    ...
    MoverParameters->BrakeLevelSet(
        MoverParameters->BrakeCtrlPos); // poprawienie hamulca po ewentualnym przestawieniu przez Pascal
    ...
    // McZapkie-250202
    iAxles = (MaxAxles < MoverParameters->NAxles) ? MaxAxles : MoverParameters->NAxles; // iloœæ osi wczytywanie z pliku nazwatypu.mmd, w tym model
...
// ***********************************************************************************************************
// Emitery dymu dla pojazdow z silnikiem Dizla
// ***********************************************************************************************************
   ...
   if (iLOADACCEPTED == 1)
    ...
    { // wyliczenie promienia z obrotów osi - modyfikacjê zg³osi³ youBy
        ...
        if ((ts.R > 15000.0) || (ts.R < -15000.0))
            ts.R = 0.0; // szkoda czasu na zbyt du¿e promienie, 4km to promieñ nie wymagaj¹cy przechy³ki
    }
    else
        ts.R = 0.0;
    ...
    { // dla EZT tylko silnikowy
        ...
        { // Ra 2013-12: to ni¿ej jest chyba trochê bez sensu
            ...
            if (v == 0.0)
            {
                ...
                if (v == 0.0)
                 if ((MoverParameters->TrainType & (dt_EZT | dt_ET40 | dt_ET41 | dt_ET42)) && MoverParameters->EngineType != ElectricInductionMotor) // dwucz³ony mog¹ mieæ sprzêg WN
    ...
    { // przeliczanie k¹tów dla pantografów
        ...
        double fCurrent = (MoverParameters->DynamicBrakeFlag && MoverParameters->ResistorsFlag ?
                 0 : fabs(MoverParameters->Itot)) + MoverParameters->TotalCurrent; // pr¹d pobierany przez pojazd - bez sensu z tym (TotalCurrent)
        ...
        { // pêtla po wszystkich pantografach
            ...
            if (MoverParameters->PantPress >
                (MoverParameters->TrainType == dt_EZT ? 2.5 : 3.3)) // Ra 2013-12: Niebugoc³aw mówi, ¿e w EZT podnosz¹ siê przy 2.5
                    ...
                    MoverParameters->PantFrontUp) // jeœli ma byæ podniesiony
            {
                ...
                { // drut siê obni¿y³ albo pantograf podniesiony za wysoko jeœli wysokoœæ jest zbyt du¿a, wyznaczyæ zmniejszenie k¹ta
...
{ // przeliczanie dŸwiêków, bo bêdzie s³ychaæ bez wyœwietlania sektora z pojazdem
    ...
    if ((MoverParameters->MainCtrlPos) >=
        (MoverParameters->TurboTest)) // hunter-250312: dlaczego zakomentowane? Ra: bo nie dzia³a³o dobrze
    {
    ...
    while (!Parser->EndOfFile && !Stop_InternalData)
    {
        ...
        if (str == AnsiString("models:")) // modele i podmodele
        {
                ...
                { // prefiks krêc¹cych siê kó³
                    ...
                    { // McZapkie-050402: wyszukiwanie kol o nazwie str*
                        ...
                        { //++iAnimatedAxles;
                            ...
                            pAnimations[i].fMaxDist *=
                                pAnimations[i].fMaxDist *
                                MoverParameters->WheelDiameter; // 50m do kwadratu, a œrednica do trzeciej
            ...
            while (!Parser->EndOfFile && str != AnsiString("endsounds"))
            {
                ...
                if (str == AnsiString("wheel_clatter:")) // polozenia osi w/m srodka pojazdu
                {
                    ...
                    if (str != AnsiString("end"))
                        str = Parser->GetNextSymbol();
                         ...
                         (MoverParameters->Power > 0)) // plik z dzwiekiem silnika, mnozniki i ofsety amp. i czest.
                {
                    str = Parser->GetNextSymbol();
                ...
                else if (str == AnsiString("brake:")) // plik z piskiem hamulca, mnozniki i ofsety amplitudy.
                {
                    str = Parser->GetNextSymbol();
                ...
                else if (str == AnsiString("brakeacc:")) // plik z przyspieszaczem (upust po zlapaniu hamowania)
                {
                    str = Parser->GetNextSymbol();
                ...
                else if (str == AnsiString("unbrake:")) // plik z piskiem hamulca, mnozniki i ofsety amplitudy.
                {
                    str = Parser->GetNextSymbol();
...
{ // zapalenie œwiate³ z przodu i z ty³u, zale¿ne od kierunku pojazdu
    ...
    if (rear == 2 + 32 + 64)
    { // jeœli koniec poci¹gu, to trzeba ustaliæ, czy jest tam czynna lokomotywa EN57 mo¿e nie mieæ koñcówek od œrodka cz³onu
...
{ // ustawienie kierunku w sk³adzie (wykonuje AI)
    ...
    if (MyTrack)
    { // podczas wczytywania wstawiane jest AI, ale mo¿e jeszcze nie byæ toru AI ustawi kierunek ponownie po uruchomieniu silnika
